...
<html lang="{{ $locale }}" dir="{{ $localeDir }}">
	<head>
		...
		@if( config('app.debug') !== true )
			<meta http-equiv="Content-Security-Policy" content="default-src 'self' data: blob:; {{ config( 'shop.csp.backend', 'style-src \'unsafe-inline\' \'self\' https://cdnjs.cloudflare.com; script-src \'unsafe-eval\' \'self\'; connect-src \'self\' https://*.deepl.com https://api.openai.com; img-src \'self\' data: blob: https://*.tile.openstreetmap.org; frame-src https://www.youtube.com https://player.vimeo.com' ) }}">
		@endif
		...
		<style nonce="{{ app( 'aimeos.context' )->get( false )->nonce() }}">
			...
			.app-menu .lang-select {height: 2.5rem; max-width: 9rem; margin: 0 0.5rem; padding: 0 0.5rem; border: 1px solid currentColor; border-radius: 0.25rem; color: var(--ai-bg); background-color: transparent; cursor: pointer}
			.app-menu .lang-select:focus-visible {outline: 2px solid currentColor; outline-offset: 2px}
		</style>
	</head>
	<body class="{{ $theme }}">
		<div class="app-menu">
